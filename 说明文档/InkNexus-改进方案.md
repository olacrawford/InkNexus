...
| 亮点 | 代码位置 | 简历参考话术 |
|---|---|---|
| AI 客服 Agent（Function Calling） | `inknexus-ai`：`BookAssistantAiService` + `QueryBookTool`/`QueryOrderTool` + `RedisChatMemoryStore` | 基于 LangChain4j + 通义千问实现电商客服 Agent，通过 Function Calling 查询图书/订单，Redis 维持多轮会话上下文 |
...
| 网关统一鉴权 | `inknexus-gateway` `AuthGlobalFilter` | 网关统一 JWT 鉴权并注入用户身份，下游服务不信任客户端请求头 |
| 全局异常 + 参数校验 | `inknexus-common` `GlobalExceptionHandler`、DTO `@Valid` | 全局异常处理与统一返回体，DTO 校验前置 |
...
1. `InkNexusRabbitMq` 新增拓扑常量；在声明类中创建：
   - 延迟队列 `inknexus.order.close.delay.queue`：`x-message-ttl` = 过期分钟数×60000，`x-dead-letter-exchange` = `inknexus.order.close.exchange`（direct，无路由键routing key直接投递）。
   - 死信队列 `inknexus.order.close.queue` 绑定到该交换机。
...
3. 压测报告整理到 `说明文档/InkNexus-压测报告.md`（场景、参数、结果表）。
...
- [ ] `mvn -f InkNexus/pom.xml -q test` 全量通过
