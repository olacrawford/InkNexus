...
`inknexus-payment` 是支付服务模块，当前使用内部模拟支付，不接真实支付宝/微信沙箱。
...
- [PaymentApplication.java](D:/workspace_idea/InkNexus/InkNexus/inknexus-payment/src/main/java/com/inknexus/payment/PaymentApplication.java)
...
数据库连接和 RabbitMQ 配置在 [nacos-config/payment.yaml](D:/workspace_idea/InkNexus/nacos-config/payment.yaml) 中维护。RabbitMQ 配置位于 `spring.rabbitmq`。
...
健康检查，返回 `inknexus-payment is running`。
...
- [OrdersView.vue](D:/workspace_idea/InkNexus/front/src/views/OrdersView.vue)：待支付订单显示“立即支付”，已支付订单支持“确认收货”
- [inknexus.js](D:/workspace_idea/InkNexus/front/src/api/inknexus.js)：`paymentApi` 请求封装
