...
这次新增了墨枢 InkNexus 前端的 Nginx 部署方案，用来模拟更贴近线上环境的访问方式。
...
文件：
- [nginx.conf](D:/workspace_idea/InkNexus/front/nginx.conf)
...
文件：
- [Dockerfile](D:/workspace_idea/InkNexus/front/Dockerfile)
...
文件：
- [docker-compose.nginx.yml](D:/workspace_idea/InkNexus/docker-compose.nginx.yml)
...
文件：
- [package.json](D:/workspace_idea/InkNexus/front/package.json)
...
至少需要：
- `inknexus-gateway` 已启动，端口 `8080`
- 相关业务微服务已启动
- Nacos、MySQL、Redis 已正常运行

### 5.2 打包前端

在 `D:\workspace_idea\InkNexus\front` 目录执行：
...
在 `D:\workspace_idea\InkNexus` 目录执行：
